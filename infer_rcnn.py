...
loader = torch.utils.data.DataLoader(pipeline, batch_size=batch_size, num_workers=num_workers, collate_fn=collate_sequences_as_dicts, shuffle=True)
...
for i, (_, data) in tqdm(enumerate(loader), total=num_samples // batch_size):
    if (split != 'test' and data['obj_pose'][0][0][0] < 0.5) or i % 1000 != 0:
        continue
