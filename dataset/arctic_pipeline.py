...
def create_pipe(in_dir, objects_root, subset, mode, device, sliding_window_size, num_seqs=4, factory=None, arctic_decoder=None):
    ...
    pipe = factory.create_datapipe(subset, num_seqs, max_concurrent_sequences=shuffle_buffer_size, shuffle_shards=True, temporal_sliding_window_size=sliding_window_size, add_component_fn=arctic_decoder if subset in ['train', 'val'] else None)
