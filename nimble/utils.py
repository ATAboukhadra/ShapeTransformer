...
def save_textured_nimble(fname, skin_v, tex_img):
    ### batch_size = 1
    import cv2
    textured_pkl = "nimble/assets/NIMBLE_TEX_FUV.pkl"
    ...
    tex_img_normalized = np.copy(tex_img)
    ...
    for line in f_uv[:-3]: # avoid last 3 elements
        if 'vt' in line:
            uv = line[:-1].split(' ')
            u = int((float(uv[1]) * 2048 - 1) / 2)
            # print(u)
            v = int((float(uv[2]) * 2048 - 1) / 2) 
            rgb = list(tex_img[u, v, :3])
            vt_rgb.append(rgb)
        if 'f' in line:
            # print(line)
            ...
            for i in range(1, 4):
                ...
                vi = int(face_side[0]) - 1
                face[i-1] = vi
                
                # Texture
                # print(face_side)
                vti = int(face_side[1]) - 1
                # if vi == 4000:
                #     print(face_side, line, face, vi, vti)
                v_rgb_map[vi] = vti
...
def write_obj(verts, faces, filename, texture=None):
    ...
    if texture is None:
        texture = np.zeros_like(verts) + 0.9
    ...
    ms.save_current_mesh(f'{filename}.obj', save_vertex_normal=True, save_vertex_color=True, save_face_color=True, save_polygonal=True)
