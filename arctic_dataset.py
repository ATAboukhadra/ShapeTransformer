...
class ArcticDataset(Dataset):
    ...
    def load_camera_matrix(self, subject, seq_name, camera, frame_num):
        ...
        if camera > 0:
            ...
            R, T = [torch.tensor(ego_annotations[f'{c}_k_cam_np'][frame_num], device=self.device) for c in ['R', 'T']]
    ...
    def load_hand_annotations(self, subject, seq_name, frame_num, cam_ext, cam_int):
        ...
        for side in ['right', 'left']:
            ...
            for component in ['rot', 'pose', 'shape', 'trans']:
                if component != 'shape':
                    comp_tensor = torch.tensor(anno[component][frame_num], device=self.device)
                else:
            ...
            pose = torch.cat((hand[0], hand[1]), dim=1)
            verts_world, kps = self.mano_layers[side](pose, hand[2], hand[3])
            verts_world /= 1000
            verts_cam = transform_points_batch(cam_ext, verts_world)
            # verts2d = project_3D_points(cam_int, verts_cam)[0].cpu().numpy()
            
            hand_dict[f'{side}_verts_world'] = verts_world[0]
            hand_dict[f'{side}_verts_cam'] = verts_cam[0]
            # hand_dict[f'{side}_verts2d'] = verts2d
    ...
    def load_obj_annotations(self, subject, seq_name, frame_num, cam_ext, cam_int, obj):
        ...
        obj_dict = {}
        articulation = torch.tensor(obj_annotations[frame_num][0], device=self.device)
        rot = torch.tensor(obj_annotations[frame_num][1:4], device=self.device)
        trans = torch.tensor(obj_annotations[frame_num][4:7], device=self.device)

        obj_dict['articulation'] = articulation
        obj_dict['rot'] = rot
        obj_dict['trans'] = trans

        z_axis = torch.FloatTensor(np.array([0, 0, -1])).view(1, 3).to(self.device)
        quat_arti = axis_angle_to_quaternion(z_axis * articulation)
        quat_global = axis_angle_to_quaternion(rot)

        for part in ['bottom', 'top']:
            if len(self.objects[obj].keys()) == 2:
                verts, faces, tex = self.objects[obj][part]
            else:
            ...
            verts = quaternion_apply(quat_arti, verts) if part == 'top' else verts
            verts = quaternion_apply(quat_global, verts) + trans 
            verts /= 1000

            verts_cam = transform_points_batch(cam_ext, verts)
            verts2d = project_3D_points(cam_int, verts_cam)[0]

            obj_dict[f'{part}_verts_world'] = verts[0]
            obj_dict[f'{part}_verts_cam'] = verts_cam[0]
            # obj_dict[f'{part}_verts2d'] = verts2d
            # obj_dict[f'{part}_faces'] = faces.verts_idx

        obj_dict['object_name'] = obj
        obj_dict['label'] = list(self.objects.keys()).index(obj)
    ...
    def __getitem__(self, idx):
        ...
        _, x, y, _, _, _ = detect_hand(img, segment=False)
        ...
        data_dict['key'] = key
        data_dict['img'] = img
        data_dict['hands_pose2d'] = hands_pose2d_placeholder
